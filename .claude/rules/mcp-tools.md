...
- **Every call passes `route: routeFor("<tool>")`** from `server/lib/routing.ts` — the only source of a worker's model and backend (`type Backend = "iu" | "max"`); never a hardcoded id. `"iu"` resolves the IU key/base via `getIuConfig()` and injects `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN`; `"max"` deletes those vars (not skips — the parent env is copied wholesale, so an inherited value would shadow OAuth) so the CLI falls through to the inherited OAuth profile, i.e. the Max subscription — a non-Claude model id is forced to `"iu"` regardless, since Max only ever serves Anthropic's own models. `ANTHROPIC_API_KEY` is always deleted regardless of backend. Every session writes a `session_env` line to `~/.claude/logs/<date>.jsonl` with `base_url` (real on `"iu"`, explicit `null` on `"max"`), `model` and `backend` — usage-tracker's classifier reads `base_url` present → `iu`, `null`/missing → `max`. A per-call `model` override goes through `withModel(route, model)`, never by hand.
...
- `readOnly: true` → `--disallowedTools "Write,Edit,NotebookEdit"` (+ `extraDisallowedTools`) so the editing tools are removed. **Never `--allowedTools`** — under `--dangerously-skip-permissions` an allowlist restricts nothing (measured on CLI 2.1.220: a probe with `--allowedTools` overwrote its canary; `tests/session-args.test.ts` pins the flag). Required for read-only tools (`check`/`review`/`overview`/`narrative`/`otel`, dispatch's read tiers) — workers edit files under skip-permissions otherwise. Bash stays available, so prompts must also say "report only"
